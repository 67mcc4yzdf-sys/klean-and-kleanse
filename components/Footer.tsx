...
export default function Footer() {
  return <footer className="bg-ink pb-24 pt-16 text-slate-300 md:pb-8"><div className="shell">
    <div className="grid gap-10 border-b border-white/10 pb-12 md:grid-cols-2 lg:grid-cols-4">
      ...
      <div><h3 className="font-bold text-white">Contact</h3><ul className="mt-5 space-y-4 text-sm"><li><a href={`tel:${business.phoneHref}`} className="flex items-center gap-3 hover:text-white"><Phone size={16} />{business.phone}</a></li><li><a href={`mailto:${business.email}`} className="flex items-center gap-3 break-all hover:text-white"><Mail size={16} />{business.email}</a></li><li className="flex items-start gap-3"><Clock size={16} className="mt-0.5 shrink-0" /><span>{business.businessHours.map(h => <span key={h} className="block">{h}</span>)}</span></li></ul></div>
